...
function setStoredCode(slug, lang, code) {
  try {
    localStorage.setItem(`code:${slug}:${lang}`, code);
  } catch {}
}
function getStoredLang(slug) {
  try {
    return localStorage.getItem(`lang:${slug}`) || "python";
  } catch {
    return "python";
  }
}
function setStoredLang(slug, lang) {
  try {
    localStorage.setItem(`lang:${slug}`, lang);
  } catch {}
}
...
function BottomPanel({
  ...
  const allPass =
    ...
    doneResults.every((r) => r !== "hidden" && r?.passed);
  const anyFail = doneResults.some(
    (r) => r !== null && r !== "hidden" && !r?.passed,
  );
  const tabColor = allPass
    ? "text-emerald-400"
    : anyFail
      ? "text-red-400"
      : doneResults.length > 0
          ...
          >
            ...
            {key === "tests" && doneResults.length > 0 && (
              <span className="ml-1.5 text-[9px]">
                {doneResults.filter((r) => r !== "hidden" && r?.passed).length}/
                {tcResults.filter((r) => r !== null).length}
              </span>
            )}
          </button>
        ))}
...
export default function Problem() {
  ...
  async function handleRun() {
    ...
    try {
      ...
      lines.push({
        type: "meta",
        text: `─── exited ${exit_code ?? 0} · ${time_ms ?? 0}ms ───`,
      });
        ...
        try {
          ...
          if (v.status === "done") {
            ...
            setConsoleLines([
              ...
              {
                type: v.verdict === "AC" ? "stdout" : "stderr",
                text: label[v.verdict] || v.verdict,
              },
    ...
    <div className="flex h-[calc(100vh-56px)] animate-fade-in overflow-hidden">
      ...
      <div className="w-[400px] flex-shrink-0 flex flex-col border-r border-border bg-bg overflow-hidden">
        <div className="px-6 pt-6 pb-4 border-b border-border flex-shrink-0">
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <span
              className={`text-xs font-semibold flex-shrink-0 mt-0.5 ${DIFF_COLOR[problem.difficulty] || "text-amber-400"}`}
            >
              {problem.difficulty}
            </span>
          </div>
                ...
                {problem.sample_cases.map((tc, i) => (
                  ...
                  >
                    <div className="grid grid-cols-2 divide-x divide-border">
                      <div className="p-3">
                        <div className="text-[10px] uppercase tracking-wider text-gray-600 mb-2 font-semibold">
                          Input
                        </div>
      ...
      <div className="flex-1 flex flex-col bg-[#0f0f0f] min-w-0">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-surface flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            {error && (
              <span className="text-xs text-red-400 font-mono max-w-[200px] truncate">
                {error}
              </span>
            )}
